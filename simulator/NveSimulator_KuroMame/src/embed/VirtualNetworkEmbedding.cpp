...
VirtualNetworkEmbedding* VirtualNetworkEmbedding::copyVirtualNetworkEmbeddingEnvironment() {
	...
	copyVNEmedding->setComVirNwNos(this->getComVirNwNos());
...
void VirtualNetworkEmbedding::addEndUserVideoRequestIntoRecords(
		map<string, EndUserRequest*> euRequest) {
	MessageSet::printInfoMessage(
			"VirtualNetworkEmbedding::addEndUserVideoRequestIntoRecords Start.");

	for (map<string, EndUserRequest*>::iterator iter = euRequest.begin();
			iter != euRequest.end(); ++iter) {

		multimap<int, EndUserRequest*>::iterator rIter =
				this->endUserRequestRecords.begin();

		this->endUserRequestRecords.insert(rIter,
				pair<int, EndUserRequest*>(iter->second->getReqEndTiming(),
						iter->second));
	}

	MessageSet::printInfoMessage(
			"VirtualNetworkEmbedding::addEndUserVideoRequestIntoRecords End.");
}
...
void VirtualNetworkEmbedding::performVirtualNetworkEnvironmentWithVirtualLinkSplittingRatio(
	...
	if (this->endUserRequestRecords.size() > 0) {
		...
		for (multimap<int, EndUserRequest*>::iterator reqIter = endReqIter.first;
			...
			amountEndingEnduserReqs++;
		}
		this->endUserRequestRecords.erase(timing);
	}
	updateCurrentAvailableEndUserRequests(timing,
			this->endUserRequestRecords.size());
	updateCurrentEndingEndUserReqs(timing, amountEndingEnduserReqs);
	...
	for (map<int, VirNwGraph*>::iterator vnwIter = this->vNws.begin();
		...
		if (vnwIter->second->getVirNwEndTiming() == timing) {
			...
			// for evaluation
			this->getComVirNwNos().erase(vnwIter->second->getVirNwNo());
	...
	updateCurrentEndingVirNwsInfo(timing, amountEndingVirNws);
...
// delete virtual network pointer;
	...
	for (map<int, VirNwGraph*>::iterator vnwIter = this->vNws.begin();
		...
		if (vnwIter->second->getVirNwEndTiming() == timing) {
			...
			//update ending virtual network amount;
			++amountEndingVirNws;
...
void VirtualNetworkEmbedding::releaseEndUserVideoRequestsForVirtualNetwork(
	...
	this->currentAvailableEndUserRequests.insert(make_pair(timing, avaAmount));

	CommonFunctions::printInfoMessage(
			" At timing: " + CommonFunctions::CoverIntegerToString(timing)
					+ ", there are "
					+ CommonFunctions::CoverIntegerToString(endAmount)
	...
	if (pathAmount == 1) {
		...
		for (multimap<int, BasePath*>::reverse_iterator pfIter =
			...
			if (kPaths_Second.size() == 0) {
				...
				kPaths = this->kShortPaths->getKShortestpathsWithHopLimit(
...
void VirtualNetworkEmbedding::updateNveDataSwitchingEnergy(const int timing) {
	...
	double comSwitchingEnergy = 0.0;
	double comDataVolume = 0.0;

	double sameSwitchingEnergy = 0.0;
	double sameDataVolume = 0.0;
	...
	for (multimap<int, EndUserRequest*>::iterator reqIter =
		...
		if (timing <= reqIter->second->getReqEndTiming()
			...
			for (map<int, VirNwGraph*>::iterator vnIter =
					this->getNws().begin(); vnIter != this->getNws().end();
					++vnIter) {
				if (reqIter->second->getReqVirNwNo()
						== vnIter->second->getVirNwNo()
						&& vnIter->second->getSameFlag() == 1) {
			...
			for (unsigned int index = 0;
				...
				if (reqIter->second->getComFlag() == 1) {
					comSwitchingEnergy =
							comSwitchingEnergy
									+ this->getSubNw()->getSubLink(
											reqIter->second->getReqSubPathsLinks()[index].first)->getSubLinkSwitchingEnergy()
	...
	// switching energy unit: joule/megabit
	switchingEnergy = switchingEnergy / 2;
	this->dataSwitchingEnergy.insert(make_pair(timing, switchingEnergy));

	comSwitchingEnergy = comSwitchingEnergy / 2;
	this->comDataSwitchingEnergy.insert(make_pair(timing, comSwitchingEnergy));

	dataVolume = dataVolume / 2;
	this->transportedDataVolume.insert(make_pair(timing, dataVolume));

	comDataVolume = comDataVolume / 2;
	this->comTransportedDataVolume.insert(make_pair(timing, comDataVolume));

	sameSwitchingEnergy = sameSwitchingEnergy / 2;
	this->sameVirNwDataSwitchingEnergy.insert(
			make_pair(timing, sameSwitchingEnergy));

	sameDataVolume = sameDataVolume / 2;
	this->sameVirNwTransportedDataVolume.insert(
			make_pair(timing, sameDataVolume));
...
const map<int, double>& VirtualNetworkEmbedding::getTransportedDataVolume() const {
	return transportedDataVolume;
}

void VirtualNetworkEmbedding::setTransportedDataVolume(
		const map<int, double>& transportedDataVolume) {
	this->transportedDataVolume = transportedDataVolume;
}

const map<int, int>& VirtualNetworkEmbedding::getCurrentAvailableEndUserRequests() const {
	return currentAvailableEndUserRequests;
}

void VirtualNetworkEmbedding::setCurrentAvailableEndUserRequests(
		const map<int, int>& currentAvailableEndUserRequests) {
	this->currentAvailableEndUserRequests = currentAvailableEndUserRequests;
}

void VirtualNetworkEmbedding::updateCurrentAvailableEndUserRequests(
		const int timing, const int amountAvailableEndUserRequests) {
	map<int, int>::iterator iter =
			this->currentAvailableEndUserRequests.begin();

	this->currentAvailableEndUserRequests.insert(iter,
			pair<int, int>(timing, amountAvailableEndUserRequests));
}

map<string, string>& VirtualNetworkEmbedding::getComVirNwNos() {
	return comVirNwNos;
}

void VirtualNetworkEmbedding::setComVirNwNos(
		const map<string, string>& comVirNwNos) {
	this->comVirNwNos = comVirNwNos;
}

void VirtualNetworkEmbedding::addComVirNwNos(const string virNwNo_1,
		const string virNwNo_2) {
	map<string, string>::iterator iter = this->comVirNwNos.begin();
	this->comVirNwNos.insert(iter, pair<string, string>(virNwNo_1, virNwNo_2));
}

/*
 * from input end-user request get common end-user request
 * , and update its request ID and virtual network No.
 */
map<string, EndUserRequest*> VirtualNetworkEmbedding::getComEndUserRequest(
		map<string, EndUserRequest*> euRequests) {
	...
	map<string, EndUserRequest*> comEuRequest;

	for (map<string, EndUserRequest*>::iterator euIter = euRequests.begin();
			euIter != euRequests.end(); ++euIter) {

		reqVirNwNo = euIter->second->getReqVirNwNo();

		for (map<string, string>::iterator cIter =
				this->getComVirNwNos().begin();
				cIter != this->getComVirNwNos().end(); ++cIter) {

			if (cIter->second == reqVirNwNo) {

				EndUserRequest* comEuReq = new EndUserRequest();
				comEuReq->copyEndUserRequests(euIter->second);

				virNo = cIter->first;
				comEuReq->setReqVirNwNo(virNo);

				reqId = euIter->second->getReqId();
				vector<string> items = CommonFunctions::stringSplit(reqId, '_');
				items[0] = virNo;

				comEuReq->setReqId(
						CommonFunctions::makeEndUserRequestId(items[0],
								items[1], items[2]));

				comEuReq->setComFlag(1);
				euIter->second->setComFlag(1);

				comEuRequest.insert(comEuRequest.begin(),
						pair<string, EndUserRequest*>(comEuReq->getReqId(),
								comEuReq));
			}
		}
	}
...
map<string, EndUserRequest*> VirtualNetworkEmbedding::removeEndUserRequestofComVirNw(
		map<string, EndUserRequest*> euRequests) {

	map<string, EndUserRequest*> euReqList;
	vector<string> virNwNos;

	/*
	 * common virtual networks
	 */
	for (map<string, string>::iterator cIter = this->getComVirNwNos().begin();
			cIter != this->getComVirNwNos().end(); ++cIter) {
		virNwNos.insert(virNwNos.begin(), cIter->first);

	}

	for (map<string, EndUserRequest*>::iterator iter = euRequests.begin();
			iter != euRequests.end(); ++iter) {

		/*
		 * find no existion items
		 */
		if (find(virNwNos.begin(), virNwNos.end(),
				iter->second->getReqVirNwNo()) == virNwNos.end()) {
			euReqList.insert(euReqList.begin(),
					pair<string, EndUserRequest*>(iter->first, iter->second));
		} else {
			delete iter->second;
		}
	}
...
const map<int, double>& VirtualNetworkEmbedding::getComDataSwitchingEnergy() const {
	return comDataSwitchingEnergy;
}

void VirtualNetworkEmbedding::setComDataSwitchingEnergy(
		const map<int, double>& comDataSwitchingEnergy) {
	this->comDataSwitchingEnergy = comDataSwitchingEnergy;
}

const map<int, double>& VirtualNetworkEmbedding::getComTransportedDataVolume() const {
	return comTransportedDataVolume;
}

void VirtualNetworkEmbedding::setComTransportedDataVolume(
		const map<int, double>& comTransportedDataVolume) {
	this->comTransportedDataVolume = comTransportedDataVolume;
}

const map<int, int>& VirtualNetworkEmbedding::getCurrentEndingVirNws() const {
	return currentEndingVirNws;
}

void VirtualNetworkEmbedding::setCurrentEndingVirNws(
		const map<int, int>& currentEndingVirNws) {
	this->currentEndingVirNws = currentEndingVirNws;
}

void VirtualNetworkEmbedding::updateCurrentEndingVirNwsInfo(const int timing,
		const int amountEndingVirNws) {
	this->currentEndingVirNws.insert(make_pair(timing, amountEndingVirNws));
}

const map<int, int>& VirtualNetworkEmbedding::getCurrentEndingEndUserReqs() const {
	return currentEndingEndUserReqs;
}

void VirtualNetworkEmbedding::setCurrentEndingEndUserReqs(
		const map<int, int>& currentEndingEndUserReqs) {
	this->currentEndingEndUserReqs = currentEndingEndUserReqs;
}
void VirtualNetworkEmbedding::updateCurrentEndingEndUserReqs(const int timing,
		const int amountEndingEuReqs) {
	this->currentEndingEndUserReqs.insert(
			make_pair(timing, amountEndingEuReqs));
}

const map<int, unsigned int>& VirtualNetworkEmbedding::getCurrentCommonEndUserReqs() const {
	return currentCommonEndUserReqs;
}

void VirtualNetworkEmbedding::setCurrentCommonEndUserReqs(
		const map<int, unsigned int>& currentCommonEndUserReqs) {
	this->currentCommonEndUserReqs = currentCommonEndUserReqs;
}

void VirtualNetworkEmbedding::updateCurrentCommonEndUserReqs(const int timing,
		const unsigned int currentCommonEndUserReqs) {

	map<int, unsigned int>::iterator iter =
			this->currentCommonEndUserReqs.begin();

	this->currentCommonEndUserReqs.insert(iter,
			pair<int, unsigned int>(timing, currentCommonEndUserReqs));

}

const map<int, pair<int, int> >& VirtualNetworkEmbedding::getCurrentSubLinkResourceInfo() const {
	return currentSubLinkResourceInfo;
}

void VirtualNetworkEmbedding::setCurrentSubLinkResourceInfo(
		const map<int, pair<int, int> >& currentSubLinkResourceInfo) {
	this->currentSubLinkResourceInfo = currentSubLinkResourceInfo;
}

void VirtualNetworkEmbedding::updateCurrentSubLinkEmbeddingResourceInfo(
		const int timing, const double minBw, const double maxBw) {

	int amountMinSubLinks = 0;
	int amountMaxSubLinks = 0;

	for (map<string, SubNwLink*>::iterator slIter =
			this->getSubNw()->getSubLinks().begin();
			slIter != this->getSubNw()->getSubLinks().end(); ++slIter) {
		if (slIter->second->getSubLinkEmbeddableBandwidth() < minBw) {
			amountMinSubLinks++;
		}

		if (slIter->second->getSubLinkEmbeddableBandwidth() < maxBw) {
			amountMaxSubLinks++;
		}
	}

	this->currentSubLinkResourceInfo.insert(
			make_pair(timing, make_pair(amountMinSubLinks, amountMaxSubLinks)));
}

const map<int, double>& VirtualNetworkEmbedding::getSameVirNwDataSwitchingEnergy() const {
	return sameVirNwDataSwitchingEnergy;
}

void VirtualNetworkEmbedding::setSameVirNwDataSwitchingEnergy(
		const map<int, double>& sameVirNwDataSwitchingEnergy) {
	this->sameVirNwDataSwitchingEnergy = sameVirNwDataSwitchingEnergy;
}

const map<int, double>& VirtualNetworkEmbedding::getSameVirNwTransportedDataVolume() const {
	return sameVirNwTransportedDataVolume;
}

void VirtualNetworkEmbedding::setSameVirNwTransportedDataVolume(
		const map<int, double>& sameVirNwTransportedDataVolume) {
	this->sameVirNwTransportedDataVolume = sameVirNwTransportedDataVolume;
}

void VirtualNetworkEmbedding::updateSameVirNwDataSwitchingEnergy(
		const int timing, const double sameVirNwDataSwitchingEnergy) {
	this->sameVirNwDataSwitchingEnergy.insert(
			make_pair(timing, sameVirNwDataSwitchingEnergy));
}

void VirtualNetworkEmbedding::updateSameVirNwTransportedDataVolume(
		const int timing, const double sameVirNwTransportedDataVolume) {
	this->sameVirNwTransportedDataVolume.insert(
			make_pair(timing, sameVirNwTransportedDataVolume));
}
