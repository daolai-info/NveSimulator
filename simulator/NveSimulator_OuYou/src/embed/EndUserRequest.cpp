...
void EndUserRequest::addReqSubPathLink(string linkId,
	...
	this->reqSubPathsLinks.push_back(pair<string,double>(linkId, allocatedDatarate));
}
...
void EndUserRequest::addReqVirPathLink(string linkId,
		double allocatedDatarate) {
	this->reqVirPathLinks.push_back(pair<string,double>(linkId, allocatedDatarate));
}
...
void EndUserRequest::printEndUserRequest(EndUserRequest* euRequest) {
	...
	for (vector<pair<string, double>>::iterator rvlIter =
		...
		cout << " virtual link: " << (*rvlIter).first << " allocated bandwidth: "
				<< (*rvlIter).second << endl;
	}
...
void EndUserRequest::addReqSubPathInfo(string pathId, double allocatedDatarate) {
	this->reqSubPathInfo.push_back(pair<string,double>(pathId, allocatedDatarate));
}
